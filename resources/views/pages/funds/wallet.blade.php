            ...
            <div class="home-content">
                ...
                <div class="wallet-hero">
                    ...
                    <div class="balance-main">
                        ₹{{ number_format(($wallet->balance ?? 0) + ($wallet->bonus_balance ?? 0), 2) }}</div>
                ...
                @if (($wallet->frozen_balance ?? 0) > 0)
                    <div class="alert alert-warning py-2 mb-3" style="border-radius:12px;font-size:12px">
                        ...
                        <strong>₹{{ number_format($wallet->frozen_balance, 2) }}</strong> is currently frozen (pending
                        withdrawal)
                    </div>
                @endif
                ...
                <div class="quick-grid">
                    <a href="{{ route('deposit.history') }}" class="quick-item">
                        <div class="qi-icon" style="background:#eff6ff"><i class="fa fa-arrow-down text-primary"></i>
                        </div>
                    ...
                    <a href="{{ route('account.statement') }}" class="quick-item">
                        <div class="qi-icon" style="background:#f0fdf4"><i class="fa fa-file-alt text-success"></i>
                        </div>
                ...
                <div class="section-head">
                    ...
                    <a href="{{ route('account.statement') }}"
                        style="font-size:12px;color:#2563eb;font-weight:600;text-decoration:none">View All</a>
                </div>
                ...
                @forelse($transactions ?? [] as $tx)
                    ...
                    @endphp
                    <div class="tx-item">
                        <div class="tx-icon {{ $isCredit ? 'credit' : 'debit' }}"><i
                                class="fa {{ $icon }}"></i></div>
                        <div class="flex-grow-1">
                            <div class="tx-desc">{{ ucfirst(str_replace('_', ' ', $tx->source ?? $tx->type)) }}</div>
                            <div class="tx-date">{{ \Carbon\Carbon::parse($tx->created_at)->format('d M Y • h:i A') }}
                            </div>
                        </div>

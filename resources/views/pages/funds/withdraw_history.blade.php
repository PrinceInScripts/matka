...
<head>
  ...
  <title>Withdrawal History | Matka Play</title>
  ...
  <style>
    ...
    .txn-amount{font-size:18px;font-weight:800;color:#ef4444;}
...
<div class="app-layout">
  <div class="left-area">
    <div class="top-bar">
      ...
      <h6 class="m-0 fw-bold">Withdrawal History</h6>
      @include('components.walletinfo')
    </div>
    <div class="home-content">
      @php
        $allW = $withdrawals->getCollection();
        $approved = $allW->where('status','approved')->sum('amount');
        $pending  = $allW->where('status','pending')->sum('amount');
        $rejected = $allW->where('status','rejected')->count();
      @endphp
      <div class="stats-row">
        <div class="stat-box"><div class="val text-success">₹{{ number_format($approved,0) }}</div><div class="lbl">Paid Out</div></div>
      ...
      <div class="txn-card {{ $w->status }}">
        <div class="txn-top">
          <div class="txn-amount">-₹{{ number_format($w->amount,2) }}</div>
          <span class="badge-pill badge-{{ $w->status }}">{{ strtoupper($w->status) }}</span>
        </div>
        <div class="txn-meta">
          <span><i class="fa fa-calendar me-1"></i>{{ $w->created_at->format('d M Y, h:i A') }}</span>
          <span><i class="fa fa-university me-1"></i>{{ $w->upi_id }}</span>
        </div>
        @if($w->admin_note)
        <div class="mt-2" style="font-size:12px;color:{{ $w->status==='rejected'?'#ef4444':'#64748b' }}">
          <i class="fa fa-comment me-1"></i>{{ $w->admin_note }}
        </div>
        @endif
        @if($w->status === 'rejected')
        <div class="mt-2" style="font-size:11px;color:#22c55e;font-weight:600">
          <i class="fa fa-undo me-1"></i>Balance restored to your wallet
        </div>
        @endif
      </div>
      @empty
      <div style="text-align:center;padding:40px;color:#94a3b8">
        ...
        <p style="font-size:14px">No withdrawal requests yet</p>
        <a href="{{ route('withdraw.funds') }}" style="color:#ef4444;font-weight:600;font-size:13px">Withdraw Now →</a>
      </div>
      @endforelse

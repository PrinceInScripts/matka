...
<head>
  ...
  <title>Gali-Disawar Result History | Admin</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    <section class="content-header"><div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6"><h1>Gali-Disawar Result History</h1></div>
        <div class="col-sm-6"><ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
          <li class="breadcrumb-item active">Gali-Disawar › Result History</li>
        </ol></div>
      </div>
    </div></section>
...
<div class="row mb-3"><div class="col-md-4"><div class="info-box bg-success">
      ...
      <table id="gdRhTable" class="table table-hover table-striped table-sm">
        <thead class="thead-dark"><tr><th>#</th><th>Game</th><th>Digit</th><th>Jodi</th><th>Panna</th><th>Status</th><th>Draw Date</th><th>Declared At</th></tr></thead>
        <tbody>@forelse($results as $r)
<tr>
  <td>{{ $loop->iteration }}</td>
  <td>{{ optional($r->gali)->name ?? '—' }}</td>
  <td><strong>{{ $r->result_digit }}</strong></td>
  <td>{{ $r->result_jodi ?? '—' }}</td>
  <td>{{ $r->result_panna ?? '—' }}</td>
...
@empty
<tr><td colspan="8" class="text-center py-3 text-muted">No results declared yet.</td></tr>
@endforelse</tbody>
      </table></div></div>
    </div></section>
  </div>
...
<script>
$(function(){
  var dt=$('#gdRhTable').DataTable({responsive:true,autoWidth:false,order:[[6,'desc']],buttons:['csv','excel','print']});
  dt.buttons().container().appendTo('#gdRhTable_wrapper .col-md-6:eq(0)');
});
</script>
</body></html>
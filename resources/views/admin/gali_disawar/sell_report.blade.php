...
<head>
  ...
  <title>Gali-Disawar Sell Report | Admin</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    <section class="content-header"><div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6"><h1>Gali-Disawar Sell Report</h1></div>
        <div class="col-sm-6"><ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
          <li class="breadcrumb-item active">Gali-Disawar › Sell Report</li>
        </ol></div>
      </div>
    </div></section>
...
$(function(){
  var dt=$('#gdSellTable').DataTable({responsive:true,autoWidth:false,buttons:['csv','excel','print']});
  dt.buttons().container().appendTo('#gdSellTable_wrapper .col-md-6:eq(0)');
  $('#btnFilter').on('click',function(){
    var btn=$(this).prop('disabled',true).html('<i class="fas fa-spinner fa-spin mr-1"></i>Loading...');
    $.post('{{ route("admin.gali_disawar.sell_report.filter") }}',{date:$('#fDate').val(), game_id:$('#fGame').val()})
    .done(function(r){
      dt.clear();

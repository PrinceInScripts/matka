...
<head>
  ...
  <title>Winning Predictions — Starline | Admin</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">    <section class="content-header"><div class="container-fluid">
      <div class="row mb-2">
        ...
        <div class="col-sm-6"><ol class="breadcrumb float-sm-right"><li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li><li class="breadcrumb-item">Starline</li><li class="breadcrumb-item active">Winning Predictions</li></ol></div>
      </div>
    </div></section>
    <section class="content"><div class="container-fluid">

<div class="card card-primary card-outline mb-3">
  <div class="card-header"><h3 class="card-title"><i class="fas fa-filter mr-2"></i>Search Winning Predictions</h3></div>
  <div class="card-body">
    <div class="row align-items-end">
      ...
      <div class="col-md-3">
        <label class="font-weight-600 small">Starline Game</label>
      ...
      <div class="col-md-2">
        <label class="font-weight-600 small">Result Digit (0–9)</label>
        <input type="number" id="pred_digit" class="form-control" min="0" max="9" placeholder="0–9">
      </div>
      <div class="col-md-3">
        <label class="font-weight-600 small">Result Panna</label>
        <select id="pred_panna" class="form-control select2">
          <option value="">— Any Panna —</option>
...
<div class="row mb-3" id="predSummary" style="display:none!important">
  <div class="col-md-3"><div class="info-box bg-primary"><span class="info-box-icon"><i class="fas fa-list-ol"></i></span>
    <div class="info-box-content"><span class="info-box-text">Matching Bids</span><span class="info-box-number" id="sPredCount">0</span></div></div></div>
...
<div class="card">
  <div class="card-header d-flex justify-content-between">
    <h3 class="card-title"><i class="fas fa-chart-line mr-2"></i>Bid Predictions — Starline</h3>
    <small class="text-muted align-self-center">Shows all bids that would win if the entered result is declared</small>
  </div>
  <div class="card-body table-responsive p-0">
    <table id="predTable" class="table table-hover table-sm mb-0">
      <thead class="thead-dark">
        <tr><th>#</th><th>User</th><th>Phone</th><th>Game Type</th><th>Panna</th><th>Bet Amount</th><th>Potential Win</th><th>Bid Date</th></tr>
      </thead>
      <tbody id="predTbody">
        <tr><td colspan="8" class="text-center py-4 text-muted"><i class="fas fa-search fa-2x d-block mb-2"></i>Enter filters above and click search</td></tr>
      </tbody>
    </table>
  </div>
</div>
...
function collectFilterParams(p){
  ...
  if($('#pred_panna').val()) p.panna=$('#pred_panna').val();
}
...
$('#predGoBtn').on('click',function(){
  ...
  var params={date:$('#pred_date').val(), starline_id:$('#pred_game').val()};
  // additional filter params filled by filter_js
  collectFilterParams(params);
  $.get('/admin/starline/winning-predictions/search',params).done(function(res){
    var tbody=''; var i=1;
    var extraCol = 'starline' === 'starline' ? 5 : 4;
    if(!res.data||!res.data.length){
      tbody='<tr><td colspan="8" class="text-center py-4 text-muted"><i class="fas fa-inbox fa-2x d-block mb-2"></i>No matching bids for given filters</td></tr>';
    }else{
      res.data.forEach(function(r){
        var gt=r.game_type?r.game_type.name:'—';
        var bv='';
        try{var bd=typeof r.bet_data==='string'?JSON.parse(r.bet_data):r.bet_data;
          bv=bd.panna||bd.digit||bd.jodi||bd.left_digit||bd.right_digit||JSON.stringify(bd);
        }catch(e){}
        tbody+='<tr><td>'+i+'</td><td><strong>'+(r.user?r.user.name:'—')+'</strong></td><td><small>'+(r.user?r.user.phone:'—')+'</small></td><td><span class="badge badge-info">'+gt+'</span></td><td><strong>'+bv+'</strong></td><td>₹'+r.amount+'</td><td class="text-success font-weight-bold">₹'+(r.winning_amount||0)+'</td><td><small>'+(r.bid_date||r.draw_date||'—')+'</small></td></tr>';
        i++;
      });
    }
    ...
    $('#sPredBid').text(tb.toFixed(2)); $('#sPredWin').text(tw.toFixed(2));
    var pnl=tb-tw;

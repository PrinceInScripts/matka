...
<body class="hold-transition sidebar-mini">
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <div class="card mt-3 d-none" id="declareSection">
                                <div class="card-header">
                                    <h3 class="card-title">Declare Starline Result</h3>
                                ...
                                <div class="card-body">
                                    <div class="row mb-3">
                                        <div class="col-md-4">
                                            ...
                                            <select id="result_digit" class="form-control select2">
                                        ...
                                        <div class="col-md-4">
                                            <label>Result Panna (Optional)</label>
                                            <select id="result_panna" class="form-control select2">
                            ...
                            <div class="card mt-4">
                                <div class="card-header">
                                    <h3 class="card-title">Starline Result History</h3>
                                </div>
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            @foreach ($results as $i => $r)
                                                <tr>
                                                    <td>{{ $i + 1 }}</td>
                                                    <td>{{ $r->starline->name }}</td>
                                                    <td>{{ $r->draw_date }}</td>
                                                    <td>{{ $r->result_digit }}</td>
                                                    <td>{{ $r->result_pana ?? '-' }}</td>
                                                    <td>
                                                        <span
                                                            class="badge badge-{{ $r->status == 'declared' ? 'success' : 'warning' }}">
                                                            {{ $r->status }}
                                                        </span>
                                                    </td>
                                                </tr>
    ...
    <script>
        ...
        $('#goBtn').on('click', function() {
            ...
            resultId = null;
            $('#result_digit').val('').trigger('change');
            $('#result_panna').val('').trigger('change');
            $('#resultStatus').text('');
            ...
            $.ajax({
                url: '/admin/starline/result/context',
                type: 'POST',
                data: {
                ...
                success: function(res) {
                    ...
                    if (res) {
                        resultId = res.id;
                        $('#result_digit').val(res.result_digit).trigger('change');
                        $('#result_panna').val(res.result_pana).trigger('change');
                        $('#resultStatus').text(res.declared_at ? 'DECLARED' : 'DRAFT');
                    } else {
                        $('#resultStatus').text('NEW');
                    }
                },
                error: function() {
                    Swal.fire('Error', 'Failed to load context', 'error');
                }
            });
        });
        ...
        $('#saveDraft').on('click', function() {
            ...
            if (!$('#result_digit').val()) {
                Swal.fire('Invalid', 'Result digit is required', 'warning');
                return;
            }
            ...
            $.ajax({
                url: '/admin/starline/result/save-draft',
                type: 'POST',
                data: {
                    starline_id: $('#game_id').val(),
                    date: $('#result_date').val(),
                    result_digit: $('#result_digit').val(),
                    result_pana: $('#result_panna').val()
                },
                success: function(res) {
         ...
         $('#showWinner').on('click', function() {
            ...
            $.ajax({
                url: `/admin/starline/result/winners/${resultId}`,
                type: 'GET',
                success: function(res) {
                    Swal.close();
                    console.log(res);
        ...
        $('#declareResult').on('click', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: 'Wallets will be credited. This cannot be undone.',
                icon: 'warning',
                ...
                $.ajax({
                    url: '/admin/starline/result/declare',
                    type: 'POST',
                    data: {
                        result_id: resultId
                    },
                    success: function() {
                        Swal.fire('Declared', 'Starline result declared successfully',
                                'success')

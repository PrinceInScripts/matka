...
<head>
  ...
  <title>Starline Bid History | Admin</title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    <section class="content-header"><div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6"><h1>Starline Bid History</h1></div>
...
<div class="card card-outline card-primary mb-3">
  ...
  <div class="col-md-3"><div class="info-box bg-warning"><span class="info-box-icon"><i class="fas fa-clock"></i></span>
    ...
    <span class="info-box-number" id="sPending">{{ $userBids->where('status','pending')->count() }}</span></div></div></div>
      ...
      <table id="slBidTable" class="table table-hover table-striped table-sm">
...
<tr>
  ...
  <td>{{ optional($b->starline)->name ?? '—' }}</td>
...
$(function(){
  var dt=$('#slBidTable').DataTable({responsive:true,autoWidth:false,order:[[10,'desc']],buttons:['csv','excel','print']});
  dt.buttons().container().appendTo('#slBidTable_wrapper .col-md-6:eq(0)');
  $('#btnFilter').on('click',function(){
    var btn=$(this).prop('disabled',true).html('<i class="fas fa-spinner fa-spin mr-1"></i>Loading...');
    $.post('{{ route("admin.starline.bid_history.filter") }}',{

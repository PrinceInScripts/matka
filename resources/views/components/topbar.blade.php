            ...
            <div class="notif-icon" id="notifBtn">
                ...
                <div class="notif-dropdown" id="notifDropdown">
                    <div class="notif-head">
                        ...
                        <span id="unreadBadge"
                            style="font-size:10px;background:#eff6ff;color:#2563eb;padding:2px 8px;border-radius:20px;"></span>
                    </div>
                    <div id="notifList" class="notif-list">
                        <div class="notif-empty"><i class="fa fa-bell-slash"></i>
    ...
    (function() {
        ...
        function typeIcon(t) {
            var m = {
                ...
                'result': 'fa-trophy text-warning',
                'bet': 'fa-gamepad text-primary',
                'notice': 'fa-bullhorn text-info'
            };
        ...
        function loadNotifs() {
            fetch('/notifications').then(r => r.json()).then(data => {
                ...
                if (!data.length) {
                    el.innerHTML =
                        '<div class="notif-empty"><i class="fa fa-bell-slash"></i><p style="font-size:12px">No notifications yet</p></div>';
                    return;
                }
                var top = data.slice(0, 5);
                var html = top.map(n => '<div class="notif-item ' + (n.is_read ? '' : 'unread') +
                    '" onclick="markRead()">' +
                    '<div class="notif-title"><i class="fa ' + typeIcon(n.type) + ' mr-1"></i>' + n
                    .title + '</div>' +
                    '<div class="notif-msg">' + n.message + '</div>' +
                    '<div class="notif-time">' + timeSince(n.created_at) + '</div></div>').join('');
                if (data.length > 5) html +=
                    '<div style="padding:7px;text-align:center;font-size:11px;color:#64748b">+' + (data
                        .length - 5) + ' more</div>';
                el.innerHTML = html;
            }).catch(() => {});
        }

...
<body>
                    ...
                    <form class="bid-form">
                        ...
                        <div class="row">
                            <div class="col-6 form-group">
                                ...
                                <input type="text" id="close-panna" name="close-panna" placeholder="Enter Close Panna" style="display:none;">
                            </div>
                            <div class="col-6 form-group">
                                <input type="text" id="close-digit" name="close-digit" placeholder="Enter Close Digit">
                                <input type="text" id="open-digit" name="open-digit" placeholder="Enter Open Digit" style="display:none;">
                            </div>
                        </div>
    ...
    <script>
        ...
        openDigit: document.getElementById("open-digit"),
        closeDigit: document.getElementById("close-digit"),
    ...
    let isOpenMode = true; 
     ...
     changeBtn.addEventListener("click", () => {
        ...
        if (isOpenMode) {
            // show open panna + close digit
            inputs.openPanna.style.display = "block";
            inputs.closeDigit.style.display = "block";
            inputs.closePanna.style.display = "none";
            inputs.openDigit.style.display = "none";
            Toastify({
                text: "Switched to OPEN PANNA & CLOSE DIGIT mode",
                duration: 2000,
                gravity: "top",
                position: "center",
            ...
            // show close panna + open digit
            inputs.openPanna.style.display = "none";
            inputs.closeDigit.style.display = "none";
            inputs.closePanna.style.display = "block";
            inputs.openDigit.style.display = "block";
            Toastify({
                text: "Switched to CLOSE PANNA & OPEN DIGIT mode",
                duration: 2000,
                gravity: "top",
                position: "center",
    ...
    function limitDigits(input, limit, label) {
        input.addEventListener("input", () => {
            ...
            if (val.length === limit) {
                ...
                input.style.borderColor = "#ffaa00"; // yellow
            } else {
    ...
    limitDigits(inputs.openDigit, 1, "Digit");
    limitDigits(inputs.closeDigit, 1, "Digit");
        ...
        let panna = isOpenMode ? inputs.openPanna.value.trim() : inputs.closePanna.value.trim();
        let digit = isOpenMode ? inputs.closeDigit.value.trim() : inputs.openDigit.value.trim();
        let points = parseInt(inputs.points.value.trim()) || 0;

        if (!panna || !digit || points <= 0) {
            Toastify({
        ...
        if (panna.length !== 3 || isNaN(panna)) {
            Toastify({
                text: "Panna must be exactly 3 numbers!",
                duration: 2500,
                gravity: "top",
                position: "center",
                backgroundColor: "#ff6b6b",
        ...
        if (digit.length !== 1 || isNaN(digit)) {
            Toastify({
                text: "Digit must be exactly 1 number!",
                duration: 2500,
                gravity: "top",
                position: "center",
                backgroundColor: "#ff6b6b",
        ...
        newRow.innerHTML = `
            <td>${panna}-${digit}</td>
            <td>${points}</td>
            <td class="game-type" style="color:#28a745">${isOpenMode ? "Open" : "Close"}</td>
        ...
        inputs.openDigit.value = "";
        inputs.closeDigit.value = "";
        inputs.points.value = "";
...
submitBidBtn.addEventListener("click", () => {
    ...
    bidTableBody.querySelectorAll("tr").forEach(row => {
        ...
        if (cols.length === 4 && !cols[0].classList.contains("text-muted")) {
            // example: "123-4" → digits = "1234" (or keep "123-4" if needed)
            const text = cols[0].innerText.trim();
            const [panna, digit] = text.includes("-") ? text.split("-") : [text, ""];
            
            allBids.push({
                digits: `${panna}-${digit}`,   // 🔹 single "digits" field for backend
                points: parseInt(cols[1].innerText),
                session: cols[2].innerText.trim().toLowerCase() // 🔹 renamed for backend
            });
        }
    });
